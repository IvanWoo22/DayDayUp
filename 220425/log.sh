...
pigz -dc CpG_sites_merge.tsv.gz |
awk -va={1} 'NR==1{for(i=1;i<=2;i++){printf \"\\t\" \$i};
for(i=a;i<=NF;i+=14){printf \"\\t\" \$i}; print \"\"};
NR>1{for(i=1;i<=3;i++) printf \$i \"\\t\";
for(i=a+1;i<=NF;i+=14) printf \$i \"\\t\"; print \$(a+14)}' > raw_L{2}_merge.tsv
" ::: {3..15..2} ::: {1..7}
...
for P in {1..15}; do
  parallel --xapply --keep-order -j 7 "awk -vk={} '{sum=0;{for(i=5;i<=17;i=i+2){if(\$i!=\"NA\"&&\$i>=15){sum++}}};{if(sum==k){print}}}' raw_P${P}_merge.tsv | wc -l" ::: {7..1}
done >each_loc_each_sample.tsv

for L in {1..7}; do
  parallel --xapply --keep-order -j 15 "awk -vk={} '{sum=0;{for(i=5;i<=NF;i=i+2){if(\$i!=\"NA\"&&\$i>=15){sum++}}};{if(sum==k){print}}}' raw_L${L}_merge.tsv | wc -l" ::: {15..1}
