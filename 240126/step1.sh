...
SCRIPT=$(realpath -s "$0")
PIPEDIR=$(dirname "$SCRIPT")
export PIPEDIR
HHDB="$PIPEDIR/pdb100_2021Mar03/pdb100_2021Mar03"

CPU="21"  # number of CPUs to use
MEM="240" # max memory (in GB)
...
function proteinMSA {
	...
	# generate MSAs
	if [ ! -s $WDIR/"$tag".msa0.a3m ]; then
		echo "Running HHblits"
		echo " -> Running command: $PIPEDIR/input_prep/make_protein_msa.sh $seqfile $WDIR $tag $CPU $MEM"
		bash "$PIPEDIR"/input_prep/make_protein_msa.sh "$seqfile" $WDIR "$tag" $CPU $MEM >$WDIR/log/make_msa."$tag".stdout 2>$WDIR/log/make_msa."$tag".stderr
	fi

	if [[ $hhpred -eq 1 ]]; then
		# search for templates
		if [ ! -s $WDIR/"$tag".hhr ]; then
