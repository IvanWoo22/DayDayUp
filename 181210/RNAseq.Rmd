...
```{r dupgenes}
gene_dup <- duplicated(genes$ENTREZID)
```
...
plotMDS(x, labels = lane, col = col.lane, dim = c(3, 4))
```
...
topTreat(tfit, coef = "BasalvsML", n = 15)
```
Now find the top 10 genes for the LP vs ML comparison.
```{r toptables_LPvsML}
topTreat(tfit, coef = "LPvsML", n = 10)
```
...
plotMD(object = tfit, coef = "BasalvsML", status = dt[, "BasalvsML"], main = "BasalvsML")
plotMD(object = tfit, coef = "LPvsML", status = dt[, "LPvsML"], main = "LPvsML")
```
...
```{r GlimmaMDplot2}
glMDPlot(tfit, coef = "BasalvsML", status = dt[, "BasalvsML"], main = "BasalvsML",
         side.main = "SYMBOL", counts = x$counts, groups = group, launch = TRUE, html = "MDplot_BasalvsML")
glMDPlot(tfit, coef = "LPvsML", status = dt[, "LPvsML"], main = "LPvsML",
         side.main = "SYMBOL", counts = x$counts, groups = group, launch = TRUE, html = "MDplot_LPvsML")
```
...
basal.vs.ml.topgenes <- topTreat(tfit, coef = "BasalvsML", n = 100)
i <- which(v$genes$ENTREZID %in% basal.vs.ml.topgenes$ENTREZID)
coolmap(lcpm[i,], labRow = v$genes$SYMBOL[i], labCol = group, keysize = 1, main = "BasalvsML")
lp.vs.ml.topgenes <- topTreat(tfit, coef = "LPvsML", n = 100)
i <- which(v$genes$ENTREZID %in% lp.vs.ml.topgenes$ENTREZID)
coolmap(lcpm[i,], labRow = v$genes$SYMBOL[i], labCol = group, keysize = 1, main = "LPvsML")
```
