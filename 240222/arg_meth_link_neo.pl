...
while (<RL>) {
    chomp;
    my @lin = split( " ", $_ );
    $chr_rl[ $cov{ $lin[0] } ]->AlignDB::IntSpan::add_pair( $lin[1], $lin[2] );
    $chr_rl[ $cov{ $lin[4] } ]->AlignDB::IntSpan::add_pair( $lin[5], $lin[6] );
}
close RL;
...
while (<LK>) {
    ...
    $set2->add_pair( $lin[5], $lin[6] );
    my $sum1 = $sit_chr[ $cov{ $lin[0] } ]->AlignDB::IntSpan::overlap($set1);
    my $sum2 = $sit_chr[ $cov{ $lin[4] } ]->AlignDB::IntSpan::overlap($set2);

    if ( ( $sum1 > $ARGV[3] ) and ( $sum2 > $ARGV[3] ) ) {
        my @intsec1 =
          $sit_chr[ $cov{ $lin[0] } ]->AlignDB::IntSpan::intersect($set1)
          ->as_array;
        my @intsec2 =
          $sit_chr[ $cov{ $lin[4] } ]->AlignDB::IntSpan::intersect($set2)
          ->as_array;
        my $meth1 = 0;
        foreach ( 0 .. $#intsec1 ) {
        ...
        my $meth2 = 0;
        foreach ( 0 .. $#intsec2 ) {
            my $tmp = $cov{ $lin[4] } . "S" . $intsec2[$_];
            $meth2 += $sit_meth{$tmp};
        }
        my $arg_meth2 = $meth2 / ( $#intsec2 + 1 );

        my $delta = $arg_meth2 - $arg_meth1;
        print OT "$_\t$delta\t$sum1\t$sum2\t$arg_meth1\t$arg_meth2\n";
    }
}
close LK;
close OT;
