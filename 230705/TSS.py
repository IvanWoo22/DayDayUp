...
def write_fasta(fasta_dict, file_handle, string):
    ''' Writes a dict of sequences into a fasta file with 60 bases per line'''

    print("Writing {}".format(file_handle))
    with open(file_handle, "w") as out:
        for chrom in fasta_dict:
            sequence = fasta_dict[chrom]
            out.write(f">{chrom}\n")
...
def normalize_bins(all_bins, methylation_bins):
    ...
    normal_all_bins = [(all_bins[x] / all_bins[x]) * 100 if all_bins[x] != 0 else 0 for x in range(len(all_bins))]
    normal_methylation_bins = [(methylation_bins[x] / all_bins[x]) * 100 if all_bins[x] != 0 else 0 for x in
                               range(len(all_bins))]
