...
my $components_file1 = $ARGV[0];
my $components_file2 = $ARGV[1];
my $input_file       = $ARGV[2];
my $components1      = READ_COMPONENTS($components_file1);
my $components2      = READ_COMPONENTS($components_file2);
...
foreach my $input (@$input_sequences) {
    ...
    if ( exists $components1->{$prefix} ) {
        ($best_set, $best_seq_name, $best_seq, $remaining_seq) =
            FIND_BEST($input->{sequence}, $components1->{$prefix});
    }
    if ( !defined($best_set) && exists $components2->{$prefix} ) {
        ($best_set, $best_seq_name, $best_seq, $remaining_seq) =
            FIND_BEST($input->{sequence}, $components2->{$prefix});
    }
    if ( defined $best_set ) {
        print "$input->{sequence}\t$input->{value}\t$best_set\t$best_seq_name\t$best_seq\t$remaining_seq\n";
    }
    else {
        print "$input->{sequence}\t$input->{value}\tNA\tNA\tNA\t$input->{sequence}\n";
    }
}
